...
/**
 * Defines a {@code UndecaFunction} that can throw an exception.
 *
 ...
 */
@FunctionalInterface
public interface ThrowableUndecaFunction<A, B, C, D, E, F, G, H, I, J, K, R> {

	/**
	 * Returns the {@code ThrowableUndecaFunction} that first executes the
	 * current {@code ThrowableUndecaFunction} instance's {@code apply} method,
	 * then uses the result as input for the {@code afterFunction} parameter's
	 * {@code apply} method.
	 *
	 * @param  throwableFunction the {@code ThrowableUndecaFunction} to execute
	 *         after the current instance
	 * @return the {@code ThrowableUndecaFunction} that executes the current
	 *         instance's {@code apply} method, then uses the result as input
	 *         for the {@code throwableFunction} parameter's {@code apply}
	 *         method
	 */
	public default <V>
		ThrowableUndecaFunction<A, B, C, D, E, F, G, H, I, J, K, V> andThen(
			ThrowableFunction<? super R, ? extends V> throwableFunction) {

		Objects.requireNonNull(throwableFunction);

		return (A a, B b, C c, D d, E e, F f, G g, H h, I i, J j, K k) ->
			throwableFunction.apply(apply(a, b, c, d, e, f, g, h, i, j, k));
	}

	/**
	 * Applies the current {@code ThrowableUndecaFunction} and returns a value
	 * of type {@code R}. This function can be implemented explicitly or with a
	 * lambda.
	 *
